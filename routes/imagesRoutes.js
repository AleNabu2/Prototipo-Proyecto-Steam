...
router.post('/upload', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const { idUser, userPassword, email, cellphone } = req.body;
    console.log('[USERNAME FIELD]', idUser);
    if (!idUser || !userPassword || !email || !cellphone) {
      console.warn('[WARN] idUser no recibido');
    ...
    console.log('[SQL] Inserting record ->', idUser, filePath);


    await db.query(
      `INSERT INTO user (idUser, userPassword, email, cellphone, image) VALUES (?, ?, ?, ?, ?)`,
      [idUser, userPassword, email, cellphone, filePath]
    );
